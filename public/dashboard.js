...
$(document).ready(() => {
  ...
  $("#btn-save-email").click(e => {
    ...
    const currentList = $(`#company-list [data-symbol="${c.data("id")}"]`);
    if (currentList.length > 0) {
      return Swal.fire({
        title: "Error",
        text: "This company is already in the list.",
        icon: "error",
        confirmButtonText: "Ok",
        closeOnConfirm: true
      });
...
$("#btn-company-delete").click(e => {
  ...
  if (currentList.length === 0) {
    return Swal.fire({
      title: "Error",
      text: "This company is not listed.",
      icon: "error",
      confirmButtonText: "Ok"
    });
  }
